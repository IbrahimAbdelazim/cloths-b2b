  ...
  return (
    <div className="relative">
      ...
      <div
        ...
        style={{ width: `calc(${(currentIndex / (STATUS_STEPS.length - 1)) * 100}% - 0%)`, right: "auto" }}
      />
        ...
        {STATUS_STEPS.map((step, i) => {
          ...
          return (
            <div key={step.status} className="flex flex-col items-center gap-2 w-16">
              <div className={`relative z-10 h-8 w-8 rounded-full border-2 flex items-center justify-center transition-all ${isDone ? "bg-indigo-600 border-indigo-600" : isCurrent ? "bg-white border-indigo-600 shadow-md shadow-indigo-100" : "bg-white border-zinc-200"}`}>
                {isDone ? (
              ...
              <span className={`text-xs text-center leading-tight ${isDone || isCurrent ? "text-indigo-700 font-medium" : "text-zinc-400"}`}>
                {step.label}
              </span>
            </div>
          );
...
export default function StoreOrderDetailClient({ id }: { id: string }) {
  const order = getOrderById(id);
  if (!order) return <div className="text-center py-20 text-zinc-400">Order not found</div>;
    ...
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-center gap-3">
        <Button variant="ghost" size="icon" asChild>
          <Link href="/store/orders"><ArrowLeft className="h-4 w-4" /></Link>
        </Button>
        <div>
          <h1 className="font-mono text-xl font-bold text-zinc-900">{order.orderNumber}</h1>
          <p className="text-sm text-zinc-500">{order.factoryName} · {placedDate.toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })}</p>
        </div>
        ...
        <CardContent className="p-6">
          ...
          {order.status !== "CANCELLED" && (
            <div className="mt-6 flex flex-col gap-1.5">
              <div className="flex items-center gap-2 text-sm">
                ...
                <span className="text-zinc-600">{placedDate.toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric" })}</span>
              </div>
              {shippedDate && (
                <div className="flex items-center gap-2 text-sm">
                  <span className="w-24 text-xs text-zinc-400 font-medium">Shipped</span>
                  <span className="text-zinc-600">{shippedDate.toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric" })}</span>
                </div>
              )}
              {deliveredDate ? (
                <div className="flex items-center gap-2 text-sm">
                  <span className="w-24 text-xs text-zinc-400 font-medium">Delivered</span>
                  <span className="text-zinc-600">{deliveredDate.toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric" })}</span>
                </div>
              ) : eta ? (
                <div className="flex items-center gap-2 text-sm">
                  <span className="w-24 text-xs text-zinc-400 font-medium">Est. Arrival</span>
                  <span className="text-zinc-600">{eta.toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric" })}</span>
                </div>
              ) : null}
            </div>
          )}
          {order.trackingNumber && (
            <div className="mt-4 p-3 bg-indigo-50 border border-indigo-100 rounded-lg flex items-center justify-between">
              <div>
                <p className="text-xs text-indigo-500 font-medium mb-0.5">{order.shippingCarrier}</p>
                <p className="font-mono text-sm font-semibold text-indigo-700">{order.trackingNumber}</p>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2">
          <Card>
            <CardHeader><CardTitle className="text-sm">Order Items ({order.items.length})</CardTitle></CardHeader>
            <CardContent className="p-0">
              ...
              <div className="px-6 py-4 border-t border-zinc-100 space-y-1.5">
                ...
                <div className="flex justify-between font-semibold text-zinc-900 pt-1 border-t border-zinc-100"><span>Total</span><span>{formatCurrency(order.totalAmount)}</span></div>
              </div>
            </CardContent>
          </Card>
        </div>
        <div className="space-y-4">
          <Card>
            <CardHeader><CardTitle className="text-sm flex items-center gap-2"><MapPin className="h-4 w-4 text-zinc-400" />Shipping Address</CardTitle></CardHeader>
          ...
          {order.buyerNote && (
            <Card>
              <CardHeader><CardTitle className="text-sm">Your Note</CardTitle></CardHeader>
              <CardContent><p className="text-sm text-zinc-600 italic">&ldquo;{order.buyerNote}&rdquo;</p></CardContent>
            </Card>
          )}
        </div>
      </div>
